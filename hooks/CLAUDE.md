# hooks - 状态管理 (L2)

> 父级: [/CLAUDE.md](../CLAUDE.md)
> 最后更新: 2026-01-31

---

## 📍 模块定位

React Hooks 集合，管理应用状态。核心是 `use-game.ts` 游戏状态机。

---

## 📁 成员清单

| 文件 | 职责 | 消费者 |
|------|------|--------|
| `use-game.ts` | 游戏状态机 | GameContainer |
| `use-mobile.ts` | 移动端检测 | 通用 |
| `use-toast.ts` | Toast 通知 | 通用 |

---

## 🔌 暴露接口

### use-game.ts

```typescript
export function useGame(): {
  // 状态
  game: GameRun | null          // 当前游戏状态
  history: GameRunSummary[]     // 本地历史记录
  
  // 生命周期
  startGame: (mode: GameMode, difficulty: Difficulty) => void
  resetGame: () => void
  clearHistory: () => void
  
  // 游戏操作
  submitAnswer: (input: string) => void
  nextQuestion: () => void
  retryQuestion: () => void
  startInput: () => void
  skipQuestion: () => void
  
  // 统计
  getStats: () => GameStats | null
}
```

---

## 🔄 状态机

```
           startGame()
               │
               ▼
┌─────────────────────────────────┐
│         GameRun                 │
│                                 │
│  runState: 'playing'            │
│       │                         │
│       ▼                         │
│  questionState 子状态机:        │
│                                 │
│  'show' ──► 'input' ──► judge   │
│                  │              │
│        ┌────────┼────────┐      │
│        ▼        ▼        ▼      │
│   'correct'  'wrong_   'wrong_  │
│      │       soft'     final'   │
│      │         │         │      │
│      │    ┌────┴────┐    │      │
│      │    ▼         ▼    ▼      │
│      │  retry    shield  │      │
│      │    │         │    │      │
│      └────┴────┬────┴────┘      │
│                ▼                │
│          nextQuestion           │
│                │                │
│    ┌───────────┴───────────┐    │
│    ▼                       ▼    │
│  继续                   finished│
└─────────────────────────────────┘
               │
               ▼
          resetGame()
               │
               ▼
           game = null
```

---

## 🎵 副作用

`use-game.ts` 内部触发的副作用：

| 事件 | 副作用 |
|------|--------|
| 答对 | playCorrectSound() |
| 连击 ≥ 2 | playComboSound(combo) |
| 速度星 | playSpeedStarSound() |
| 护盾激活 | playShieldSound() |
| 答错 | playWrongSound() |
| 游戏结束 | playFinishSound() |
| 限时模式 | setInterval 倒计时 |

---

## ⚠️ 技术债

| 问题 | 影响 | 解决方案 |
|------|------|----------|
| 单 useState 管理复杂状态 | 状态更新可能不一致 | 改用 useReducer 或 XState |
| 仅本地历史 | 跨设备丢失 | 接入 DB + Auth |
| 音效副作用内联 | 不可配置/关闭 | 抽离为可配置服务 |

---

## 🔗 依赖关系

```
hooks/use-game.ts
    ├── 依赖 lib/game-types.ts (类型 + 配置 + 计算函数)
    ├── 依赖 lib/question-generator.ts (题目生成)
    └── 依赖 lib/sounds.ts (音效播放)
```

---

## 📋 变更日志

| 日期 | 变更 | 文件 |
|------|------|------|
| 2026-01-31 | 初始实现 | use-game.ts |
| 2026-01-31 | 增加本地历史持久化 | use-game.ts |
| 2026-01-31 | 创建模块文档 | CLAUDE.md |

---

[PROTOCOL]: 变更时更新此文档，然后检查 /CLAUDE.md
